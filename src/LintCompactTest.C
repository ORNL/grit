...
int main(int argc, char *argv[]){
  ...
  Lint<DustTest> Subset = Parcels.extract(DustTest::HEALTHY);

  printf("Lint Compact Test extract subset %4zu parcels with fill percentages = ", Subset.size());
  for(DustTest P: Subset ) printf("%9.6f ", 1.0-float(P.getcount(DustTest::UNOCCUPIED))/DustTest::NDUST);
  printf("\n");
  Subset.write_silo("Subset");
